...
interface BookingDao {
    @Query("SELECT * FROM bookings WHERE syncStatus != 'PENDING_DELETE' ORDER BY date DESC, startTime DESC")
    fun getVisibleBookingsFlow(): Flow<List<BookingEntity>>

    @Query("SELECT * FROM bookings ORDER BY date DESC, startTime DESC")
    suspend fun getAllBookings(): List<BookingEntity>

    @Query("SELECT * FROM bookings WHERE syncStatus != 'PENDING_DELETE' ORDER BY date DESC, startTime DESC")
    suspend fun getVisibleBookings(): List<BookingEntity>
